...
(function() {
  jQuery(function($) {
    ...
    nav = {
      ...
      scroll_out: function() {
        ...
        return $(".nav-container").animate({
          ...
          return $(".close-nav-touch").show();
        });
      }
    };
    $(document).on('click', '.open-nav-slider', function(event) {
      return nav.scroll_out();
    });
    $(document).on('click', '.close-nav-slider', function(event) {
      return nav.scroll_in();
    });
    $(document).ready(function(event) {
      var name, screen_width, tab;
      name = window.location.pathname;
      tab = $('a[href="' + name + '"]');
      tab.addClass('active');
      screen_width = $(window).innerWidth();
      if (!(screen_width < 1200 && screen_width > 500)) {
        tab.parents('.dropdown').show();
        return $.each(tab.parents('.dropdown').siblings('.dropdown-toogle'), function(index, obj) {
          $(obj).addClass('active');
          return $(obj).attr('data-open', 'true');
        });
      }
    });
    $('.main-nav').on('click', '.dropdown-toogle', function(event) {
      var id, open;
      id = $(this).attr('id');
    ...
    return $('.nav-container').on('click', '.close-nav-touch', function(event) {
      return nav.scroll_in();
    });
  });
